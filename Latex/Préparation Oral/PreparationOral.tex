...
% Définir les pieds de page pour les pages impaires (RO) et paires (LE)

\pagestyle{fancy} 
\fancyhead{}
\fancyhead[RE]{\textit{Préparation à l'oral - $M\pi E$ Wallon - Physique-Chimie}}
%
...
\gauchedroite{Physique-Chimie }{$\text{M}\pi\text{E}$ Wallon}\par
\begin{center}
    \LARGE{\textbf{Préparation à l'Oral}}
\vspace{0.5em}
\end{center}
\end{tcolorbox}
...
\begin{minipage}[t]{0.8\textwidth} % Colonne de gauche (48% de la largeur du texte)

Vous trouverez dans ce fascicule des recueils d’exercices d’oraux des dernières années, classés par Thème et par Concours.

Avant d’entamer un exercice, assurez-vous tout d’abord d’avoir révisé le cours associé.

Lorsque le logo \indications \hspace{0.5em}  apparaît, c’est que vous pourrez trouver des indications, en cas de blocage lors de la recherche de cet exercice, sur le fichier pdf « Indications ».

Lorsque le logo \corrige \hspace{0.5em} apparaît, c’est que :
\begin{itemize}
\item	Vous pourrez trouver les éléments de solution sur le fichier pdf « Indications ». 
\item Vous pourrez obtenir un corrigé détaillé sur demande par mail.  \textit{
Certains de ces corrigés sont d’ores et déjà présents sur le Drive. Les autres seront mis après la séance de TD consacré aux thèmes des exercices en question.}
\end{itemize}
\end{minipage}
\hfill % Espace entre les colonnes
\begin{minipage}[t]{0.15\textwidth} % Colonne de droite (48% de la largeur du texte)
\vspace{0.5em}
 \includegraphic{}{images/einstein.png}
\end{minipage}
...
Nous traiterons une partie de ces exercices lors des séances de TD. Je vous encourage à en chercher davantage de manière autonome ou en petits groupes. Toutefois… ne cherchez dans ce cas que ceux dont la présence d’un corrigé est signalée : il ne sert à rien de chercher un sujet si vous n’avez pas la possibilité de vous auto-corriger ensuite. Il est possible également de chercher un sujet pour lequel la présence d’un corrigé n’est pas stipulée… mais dans ce cas, il faudra me rendre une version papier de votre corrigé pour que je vérifie ce que vous avez fait.

Par ailleurs, les feuilles de TD de l’année regorgent d’exos d’oraux corrigés, parfois avec indications. Si vous souhaitez réviser en particulier certains cours, n’hésitez pas non plus à retourner les étudier.


\begin{bclogo}[logo=\bcoutil , noborder=True, couleur=yellow!10!white]{\hfill{Les 5 étapes indispensables du travail des exercices}}

\begin{itemize}

\item[$\bullet$] Relire et s’assurer de maîtriser le cours associé à l’exercice

\item[$\bullet$] Chercher l’exercice seul

\item[$\bullet$] En cas de blocage, consulter les indications ou demander conseil. Si le blocage persiste, insister et consulter le corrigé détaillé.

\item[$\bullet$] Vérifier ses résultats à l’aide du corrigé détaillé ou lors de la séance de correction en classe.

\item[$\bullet$] Prendre du recul et dégager la problématique et la structure de l’exercice.
...
Lien vers le dossier avec le fichier d’Indications et les Corrigés :

\begin{flushright}
\includegraphic{0.2}{images/qrcode.png}
\end{flushright}
